...
namespace HoloToolkit.Examples.SpatialUnderstandingFeatureOverview
{
    public class AppState : Singleton<AppState>, ISourceStateHandler, IInputClickHandler
    {
        ...
        public bool gameClean = false;
        public int playOnlyOnce = 1;
        public AudioSource goBacksound;
        // Config
        ...
        public string PrimaryText
        {
            get
            {
                ...
                if (SpatialUnderstanding.Instance.AllowSpatialUnderstanding)
                {
                    switch (SpatialUnderstanding.Instance.ScanState)
                    {
                        case SpatialUnderstanding.ScanStates.Scanning:
                            ...
                            if (DoesScanMeetMinBarForCompletion)
                            {
                                ...
                                playOnlyOnce = 0;
                                return "Une fois la zone scannée, air-tap";
                            }
                            return "Deplacez vous et scannez la salle";
                        case SpatialUnderstanding.ScanStates.Finishing:
                            PlayerPrefs.SetInt("isScanningDone", 1);
                            return "Merci de patienter, finalisation du scan";
                        case SpatialUnderstanding.ScanStates.Done:
                            if (!gameClean)
                            {
                                GameObject sfplane = GameObject.FindGameObjectWithTag("Planes");
                                sfplane.SetActive(false);

                                GameObject sfunderstanding = GameObject.FindGameObjectWithTag("gameObjectToDelete");
                                sfunderstanding.SetActive(false);
                                gameClean = true;
                            }
                    ...
                    //default : return "ScanState = " + SpatialUnderstanding.Instance.ScanState.ToString();
                }
                return "";
            }
        }
        ...
        public Color PrimaryColor
        {
            get
            {
                ...
                return (!string.IsNullOrEmpty(SpaceQueryDescription) || !string.IsNullOrEmpty(ObjectPlacementDescription)) ?
                    (PrimaryText.Contains("Patientez") ? new Color(1.0f, 0.0f, 0.0f, 1.0f) : new Color(1.0f, 0.7f, 0.1f, alpha)) :
